...
class VOCMerge(data.Dataset):
    def __init__(self, root, image_set, dataset_name='merge_voc', *args, **kwargs):
        ...
        for id, item in enumerate(label_map.item, 1):
            try:
                sub_class = item.name
                self.sub_class_dict[sub_class] = VOCDetection(os.path.join(root, dataset_name), sub_class + '_' + image_set,
        ...
        for sub_class, sub_class_ds in self.sub_class_dict.items():
            sub_class_ds.label_map_dict = label_map_dict
