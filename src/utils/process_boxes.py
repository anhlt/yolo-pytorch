...
def preprocess_true_boxes(true_boxes, anchors, image_size: Tuple[int, int], num_classes: int, ratio=32):
    ...
    num_classes : int
        Description
    ratio : int, optional
        Ratio between origin size and feature map
    ...
    assert height % ratio == 0, 'Image sizes in YOLO_v2 must be multiples of %d.' % ratio
    assert width % ratio == 0, 'Image sizes in YOLO_v2 must be multiples of %d.' % ratio
    conv_height = height // ratio
    conv_width = width // ratio
