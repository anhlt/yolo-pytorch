...
class AttentionConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        for kernel_size in kernel_sizes:
            padding = int((kernel_size - 1) / 2)  # always same padding
...
class ConvAttention2DSize3(nn.Module):
    ...
    def forward(self, x):
        conv_out = self.unfold(x)
        N, C, H, W = conv_out.shape
        multi_head_in = conv_out.permute(0, 2, 3, 1).contiguous().view(N * H * W, self.number_of_kernel, -1)
        out = self.multi_head_attention(multi_head_in, multi_head_in, multi_head_in)
...
class ConvAttention2DSize5(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.multi_head_attention(multi_head_in, multi_head_in, multi_head_in)
        out = out.view(N, H, W, -1).permute(0, 3, 1, 2)
        return out[:, self.out_channels: self.out_channels * 2, :, :]
